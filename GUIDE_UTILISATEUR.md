...
```bash
curl https://finance-buddy-308.preview.emergentagent.com/api/export/all > backup.json
```

### 4. Import via API
```bash
curl -X POST https://finance-buddy-308.preview.emergentagent.com/api/import/all \
...
- ✅ Déjà déployé sur : https://finance-buddy-308.preview.emergentagent.com
...
```bash
# 1. Tester le backend
curl https://finance-buddy-308.preview.emergentagent.com/api/

# Devrait retourner :
# {"message":"FinanceApp API v1.0","status":"operational", ...}

# 2. Créer un compte de test
curl -X POST https://finance-buddy-308.preview.emergentagent.com/api/accounts \
  -H "Content-Type: application/json" \
  -d '{
...
# Ouvrir https://finance-buddy-308.preview.emergentagent.com
# Cliquer sur "Comptes"
