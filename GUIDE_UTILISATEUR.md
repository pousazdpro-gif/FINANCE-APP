...
```bash
curl https://budgetbrain.preview.emergentagent.com/api/export/all > backup.json
```

### 4. Import via API
```bash
curl -X POST https://budgetbrain.preview.emergentagent.com/api/import/all \
...
- ✅ Déjà déployé sur : https://budgetbrain.preview.emergentagent.com
...
```bash
# 1. Tester le backend
curl https://budgetbrain.preview.emergentagent.com/api/

# Devrait retourner :
# {"message":"FinanceApp API v1.0","status":"operational", ...}

# 2. Créer un compte de test
curl -X POST https://budgetbrain.preview.emergentagent.com/api/accounts \
  -H "Content-Type: application/json" \
  -d '{
...
# Ouvrir https://budgetbrain.preview.emergentagent.com
# Cliquer sur "Comptes"
